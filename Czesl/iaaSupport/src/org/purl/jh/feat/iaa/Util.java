...
public class Util {
        /** todo utils */
    public static WLayer getWLayer(FormsLayer<?> layer) {
        for (;;) {
            if (layer == null || layer instanceof WLayer) return (WLayer)layer;
    ...
    public static Collection<WForm> getWForms(Edge aEdge) {
        ...
        for (;;) {
            ...
            lowerForms = getLowerForms(lowerForms);
        }
    }
    
// TODO TEMPORARY (fROM FeatDataUtil - they are not accessible    
    ...
    public static int maxLen(Iterable<String> strs) {
        int max = 0;
